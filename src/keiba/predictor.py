...
# デフォルトの重み（スピード指数を主軸に、追切・血統・騎手厩舎で補正）
# - 追切0.2は坂路好タイム1年分のバックテスト(3,063レース)での実測最適値
# - 脚質×コース形態は重み0(アブレーションで学習/検証とも除去した方が
#   全指標が改善、重賞では単回収+10pt)。偏差値は参考表示のみ残す
DEFAULT_WEIGHTS = {
    "speed": 0.5, "workout": 0.2, "pedigree": 0.2, "style": 0.0,
    "connections": 0.1,
}
