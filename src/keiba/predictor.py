...
# デフォルトの重み（スピード指数を主軸に、追切・騎手厩舎・血統で補正）
# ローリング・ウォークフォワード(毎年、過去データのみで重みを選び翌年で検証、
# 2023-2026の4年11,848レース)で毎年ほぼ同一の構成が選ばれた的中率最適値:
# - 騎手厩舎0.2(旧0.1から倍増)・血統0.1(旧0.2から半減)で◎複勝率+2.5pt
# - トレードオフ: 血統を軽くした分、単複回収率は1〜2pt低下(的中率を優先)
# - 脚質は重み0(アブレーション・ウォークフォワードとも不要判定、表示のみ)
# 追切0.1: 調教データが2022年まで遡れるようになって初めてローリング検証でき、
# 0.0が最良・上げるほど単調悪化と判明した(4年12,734R・全Foldで同傾向、
# 好タイム限定の母集団でも同じ → scripts/rolling_workout_weight.py)。
# 0.0との差は0.1〜0.3ptで実質同等のため、検証に含められない情報
# (記事の脚色・併せ結果。索引は全馬「馬なり」として評価される)の分だけ
# 0.1を残している。旧値0.2は坂路好タイム1年分の実測値だった。
DEFAULT_WEIGHTS = {
    "speed": 0.5, "workout": 0.1, "pedigree": 0.1, "style": 0.0,
    "connections": 0.2,
}

# 当日馬場が良以外のとき、道悪適性を追加の要素として自動追加する重み
# （他の重みと合算後に正規化されるため、渋るほど道悪適性の比重が上がる）
# 旧値(0.15/0.25/0.30)はアブレーションで過剰と判明し2/3に縮小
WET_FACTOR_WEIGHTS = {"稍重": 0.10, "重": 0.17, "不良": 0.20}
